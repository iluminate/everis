#! /bin/bash
#************************************************************************
#* PROGRAMA		: reporteVentasBacklog.sh				*
...
DIRTEMP=${DIR}/temp
FILETEMP=${DIRTEMP}/reporteVentasBacklog_${TODAY}.temp
DIRDATA=${DIR}/data
FILEJSON=${DIRDATA}/reporteVentasBacklog_${TODAY}.json
FILEDATA=${DIRDATA}/reporteVentasBacklog_${TODAY}.dat
# JIRA_QUERY="project%20in%20(EAI%2COSB%2COSB_MR%2CTIM-FTP1%2CPROCPP_MIG%2CJANUS%2CSGA%2CSIGPEL)%20AND%20Proveedor%3DEveris%20AND%20status%20in%20(Analisis%2CBacklog)%20AND%20%22Analista%20Programador%22%20is%20EMPTY"
JIRA_QUERY="project%20in%20(SISACT%2CSICAR%2CSISACT_PREPAGO%2CSISATC%2CSISCAD)%20AND%20Proveedor%3DEveris%20AND%20status%20in%20(Analisis%2CBacklog)"
...
if [ -s ${FILETEMP} ]; then
mail -a "MIME-Version: 1.0" -a "Content-Type: text/html" -s "${VENTAS_MAIL_SUBJECT}" "${VENTAS_MAIL_TO}" -c "${VENTAS_MAIL_CC}" <<EOF
